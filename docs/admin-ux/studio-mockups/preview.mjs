...
if (serve) console.log(`Mockup preview: ${origin}`);
  ...
  try {
    ...
    await page.locator('[data-document="Fit analysis"] summary').focus();
    await page.keyboard.press('Enter');
    assert.equal(await page.locator('.version-dialog').isVisible(), false);
    assert.equal(await page.locator('[data-document="Fit analysis"] .version-row:visible').count(), 3);
    await page.keyboard.press('Enter');
    assert.equal(await page.locator('[data-document="Fit analysis"] .version-row:visible').count(), 0);
    await page.keyboard.press('Enter');
    await page.screenshot({ path: fileURLToPath(new URL('05-document-versions.png', import.meta.url)) });
    await page.locator('[data-document="Fit analysis"] [data-preview-version="2"]').click();
    await page.locator('.version-dialog [data-publish-version="2"]').click();
    ...
    assert.ok(await page.locator('[data-document="Fit analysis"] .version-row').first().textContent().then(s => s.includes('Astra') && s.includes('Latest draft')));
    assert.ok(await page.locator('[data-document="Fit analysis"] .version-row.live').textContent().then(s => s.includes('Version 2')));
    await page.keyboard.press('Escape');
    ...
    await page.locator('[data-create-version]').click();
    await page.locator('[data-document="Cover letter"] [data-publish-version="1"]').click();
    await page.keyboard.press('Escape');
