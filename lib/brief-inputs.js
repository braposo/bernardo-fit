...
export function briefInputs(job, report, research) {
  return {
    ...
    personalFacts: {
      ...
      availability: analysisSettings().personalFacts.availability, location: clean(job.location || analysisSettings().personalFacts.location, 300), locationMode: clean(job.locationMode, 200),
      noticePeriod: confirmed(job, /notice\s*period/i), startDate: confirmed(job, /start\s*date/i),
      sponsorship: analysisSettings().personalFacts.sponsorship,
    },
    research,
  };
}
