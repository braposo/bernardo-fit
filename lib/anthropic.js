...
export async function complete({
  ...
  const prompt = promptIdentity(kind, system.stable, analysisSettings().revision);
  ...
  for (let turn = 0; turn <= maxContinuations; turn++) {
    ...
    const { response, data: responseData } = await traceModelRequest({ provider: "anthropic",
      model: resolvedModel, kind, attempt: generationAttempt, continuation: turn, maxTokens, prompt, requestBody }, () => fetch(ENDPOINT, {
      method: "POST",
      headers: {
