...
function reviewCopy(kind, job, { researchAction = "none", count = 0 } = {}) {
  ...
  return { ...common, subject: count + " role" + (count === 1 ? "" : "s"), title: "Analyse listed roles",
    submitLabel: "Analyse " + count + " role" + (count === 1 ? "" : "s"),
    description: "We’ll create or update the fit page for each listed role below, showing where your experience aligns and where there may be gaps.",
    publication: "Each completed analysis becomes that role's active fit page. Roles outside this list are not included." };
}
...
export async function resolveGenerationReview(body = {}) {
  ...
  if (requestedKind === "analyse-all") {
    ...
    if (!Array.isArray(requestedIds) || !requestedIds.length || requestedIds.length > 500 ||
      ...
      throw Object.assign(new Error("Choose roles from the current list before reviewing analysis."), { status: 400 });
    }
    const selected = new Set(requestedIds);
    const candidates = (await listJobs({ includeArchived: true }))
      .filter((job) => selected.has(job.id) && (job.fitReportId || String(job.jobDescription || "").trim().length >= 20))
      .sort((a, b) => String(a.id).localeCompare(String(b.id)));
    if (candidates.length !== selected.size) throw conflict("The listed roles changed or need fuller descriptions. Refresh the list and review again.");
    const eligible = [];
    for (let i = 0; i < candidates.length; i += 4) {
      eligible.push(...await Promise.all(candidates.slice(i, i + 4).map(async job => {
        ...
        if (job.fitReportId && !report) throw conflict("A listed analysis is unavailable. Refresh the list and review again.");
        const mode = report ? "replace" : "create";
        const routing = await selectWritingModel({ kind: mode === "replace" ? "regenerate" : "analyse", job, report, fallback: model });
        return { id: job.id, company: job.company || "", role: job.role || "", model: routing.model, routing,
          mode, reportId: report ? job.fitReportId : "", fingerprint: analysisFingerprint(job, routing.model, mode, report) };
      })));
    }
    ...
    return { kind: requestedKind, model, fingerprint, workFingerprint, payload: { model, jobs: eligible.map(({ id, fingerprint, model, mode, reportId }) => ({ id, fingerprint, model, mode, reportId })) },
