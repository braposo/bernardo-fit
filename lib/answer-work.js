...
export async function executeAnswerWork({ jobId, questionId, requestId, fingerprint, model, economy = false, modelRouting = false }) {
  ...
  if (question.run?.requestId !== requestId || answerFingerprint(job, questionId, wanted, report, { economy }) !== fingerprint) {
  ...
  if (!result) {
    ...
    result = await saveTaskResult("answer", jobId, requestId, await runAnswer({
      ...
      previous, instructions: job.instructions, model: wanted, ref: requestId, economy, modelRouting,
    }));
  }
  const latest = await getJob(jobId);
  const latestReport = latest?.fitReportId ? await getReport(latest.fitReportId) : null;
  if (!latest || answerFingerprint(latest, questionId, wanted, latestReport, { economy }) !== fingerprint) {
    await mark(jobId, questionId, requestId, "superseded");
    return { outcome: "superseded", jobId, questionId, requestId };
  }
  ...
  return { outcome: "completed", jobId, questionId, requestId, refused: result.refused,
    words: result.words, limit: result.limit, over: result.over, model: result.model ?? wanted, source: result.source || "model", routing: result.routing || "existing-policy" };
}
