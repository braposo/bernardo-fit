...
export async function executeAnswerWork({ jobId, questionId, requestId, fingerprint, model, economy = false }) {
  ...
  if (!result) {
    ...
    result = await saveTaskResult("answer", jobId, requestId, await runAnswer({
      ...
      previous, instructions: job.instructions, model: wanted, ref: requestId, economy,
    }));
  }
