...
export async function evaluateJev({ state, questions, kind, ref, signal: callerSignal, model = JEV_MODEL }) {
      ...
      const result = await logger.trace("chat " + model, async span => {
        ...
        try {
          ...
          try {
            ...
            if (data?.model) attribute("gen_ai.response.model", data.model);
