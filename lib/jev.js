...
export const jevEnabled = (env = process.env) => env.JEV_ENABLED === "1" && !!env.AI_GATEWAY_API_KEY?.trim();
...
export async function evaluateJev({ state, questions, kind, ref }) {
  if (!jevEnabled()) throw Object.assign(new Error("Enable Jev and configure AI_GATEWAY_API_KEY in Vercel and Trigger.dev."), { status: 503, abort: true });
