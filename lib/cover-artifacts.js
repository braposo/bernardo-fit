...
import { COVER_INDEX_WRITE, ARTIFACT_CREATE } from "./store-scripts.js";
...
function metadata(artifact) {
  return {
    ...
    versionInstructions: artifact.versionInstructions || "",
...
export async function getCoverArtifact(jobId, vid) {
  ...
  if (sanityStorageEnabled()) return repository().getArtifact('cover',jobId,vid);
  return hasKV
...
export async function saveCoverArtifact(jobId, value, { preserveId = "" } = {}) {
  ...
  if (sanityStorageEnabled()) {
    const saved=await repository().saveArtifact('cover',jobId,artifact.vid,artifact);
    return {artifact:saved,versionCount:(await repository().listArtifacts('cover',jobId)).length};
  }
  const key = artifactKey(jobId, artifact.vid);
  const event = auditEvent("document.generated", "Cover letter version saved", [artifact.model, artifact.vid].filter(Boolean).join(" · "), { id: `cover-${artifact.vid}`, at: artifact.at });
  if (hasKV) await (await kv()).eval(ARTIFACT_CREATE, [key, auditKey("job", jobId)], [JSON.stringify(artifact), JSON.stringify(event), event.timestamp]);
  else if (!memoryArtifacts.has(key)) {
    memoryArtifacts.set(key, clone(artifact));
...
function legacyArtifacts(job, { preserveMissingDates = false } = {}) {
  const versions = (job.coverLetterVersions || [])
    .filter((v) => v?.vid && Array.isArray(v.paragraphs) && v.paragraphs.length)
    .map((v) => ({ ...normaliseArtifact(job.id, v), ...(preserveMissingDates ? { at: v.at || "" } : {}) }));
  if (Array.isArray(job.coverLetter) && job.coverLetter.length) {
    ...
    if (!matched) {
      ...
      }), ...(preserveMissingDates ? { at: job.coverLetterAt || "" } : {}) });
    }
  }
  return versions;
}
...
export async function listCoverVersions(job, options) {
  if (sanityStorageEnabled()) return (await repository().listArtifacts('cover',job.id)).map(v=>({...metadata(v),active:v.vid===job.coverLetterId}));
  ...
  const legacy = legacyArtifacts(job, options);
...
export async function deleteCoverArtifacts(jobId) {
  if (sanityStorageEnabled()) return repository().deleteArtifacts('cover',jobId);
