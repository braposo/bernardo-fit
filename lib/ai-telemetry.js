...
import { setModelContentAttributes } from "./gen-ai-content.js";
...
export async function traceModelRequest({ provider, model, kind, attempt, continuation, maxTokens, prompt, requestBody }, request) {
  return logger.trace("chat " + model, async span => {
    ...
    try {
      ...
      try {
        ...
        if (requestBody) setModelContentAttributes(attribute, { provider, request: requestBody, data });
