...
import { toolResultErrors } from "./anthropic.js";
...
export async function runResearch({ job, model = PUBLIC_MODEL, ref }) {
  ...
  const attempts = ref ? (await readUsage(ref)).filter(entry => entry.kind === "research") : [];
  const remainingSearches = MAX_SEARCHES - attempts.reduce((sum, entry) => sum + (entry.searches || 0), 0);
  const remainingCalls = 3 - attempts.length;
  if (remainingSearches <= 0 || remainingCalls <= 0) {
    throw Object.assign(new Error("Company research has reached this request's budget. Start a new request to try again."), { status: 502, abort: true });
  }
  const result = await complete({
    model, effort: "medium", maxTokens: 8192, kind: "research", ref,
    maxContinuations: remainingCalls - 1, maxSearches: remainingSearches,
    ...
    system: { stable: renderPrompt('research'), volatile: job._versionInstructions ? instructionsBlock(job.instructions) : "" },
    messages: [{ role: "user", content: JSON.stringify(input) }],
  });
  const raw = parseLooseJson(result.text, (v) => v && typeof v === "object");
  if (!raw) throw Object.assign(new Error("Company research did not return valid structured data."), { status: 502, abort: true });
