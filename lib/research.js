...
export function companyResearchInput(job) {
  let domain = "";
  try { domain = new URL(job.sourceUrl || "").hostname.replace(/^www\./, ""); } catch {}
  return { company: text(job.company, 200), domain, role: text(job.role, 200), searchBudget: MAX_SEARCHES };
}
...
export function normaliseResearch(raw, blocks, input, { at = new Date().toISOString(), model = PUBLIC_MODEL, partial = false } = {}) {
  ...
  return {
    ...
    unknowns: (Array.isArray(raw?.unknowns) ? raw.unknowns : []).slice(0, 15).map((v) => text(v, 500)).filter(Boolean),
    sources,
  };
}
