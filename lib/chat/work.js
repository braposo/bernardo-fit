...
export function createChatWork({ connect = connectContext, select = selectChatModel,
  ...
  return async function execute({ request: input, requestId: ref }, { signal: cancellation, emit = async () => {} } = {}) {
    ...
    try {
      ...
      await phase('routing');
      route = await select({ ...request, provider: 'auto', model: 'auto' }, { env, ref, signal });
      await emit('route', { requestId: ref });
      const response = await makeAgent({ route, context, ref }).stream({ messages: request.messages, abortSignal: signal });
      let finish;
      ...
      result.error = cancellation?.aborted ? '' : signal.aborted ? 'The response timed out. Try a shorter question.' :
        error.code === 'CHAT_RATE_LIMIT' ? error.message : 'Chat could not complete the response. Please retry.';
    } finally {
