...
export function createChatWork({ connect = connectContext, select = selectChatModel,
  ...
  return async function execute({ request: input, requestId: ref }, { signal: cancellation, emit = async () => {}, onRoute = () => {} } = {}) {
    ...
    try {
      ...
      // Some providers omit a tool-result stream part even after its execute callback completed.
      result.sources = [...context.sources.values()];
      if (result.sources.length) await emit('sources', { sources: result.sources });
