...
export async function saveChatTurn({ request, ref, route, text, outcome, env = process.env,
  ...
  await client.context.conversations.save({
    ...
    metadata: { mcpEndpoints: [endpoint], conversationId: request.conversationId || ref,
      outcome, environment: env.VERCEL_ENV || (env.TRIGGER_SECRET_KEY?.startsWith('tr_prod_') ? 'production' : 'development'), policy: route?.policy || "unknown",
      ...(route?.settingsRevision ? {settingsRevision:route.settingsRevision,settingsFingerprint:route.settingsFingerprint} : {}) },
