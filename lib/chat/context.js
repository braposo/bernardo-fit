...
const SOURCE_METADATA_QUERY = `*[_id in $ids && _type in $types && !defined(deletedAt) && pending != true]{
  _id, _type, title, role, name, question, jobTitle, legacyId, "job": job->{legacyId}
}`;
...
export function collectSources(result, sources) {
  ...
  function visit(value, depth = 0) {
    if (!value || typeof value !== "object" || depth > 20 || sources.size + unresolved.size >= 40) return;
    if (!Array.isArray(value) && CHAT_TYPES.includes(value._type) && typeof value._id === "string") {
      const title = value.title || value.role || value.name || value.question || value.jobTitle || value._type;
      const jobId = value._type === "job" ? value.legacyId : value.job?.legacyId;
      sources.set(value._id, { id: value._id, type: value._type, title: String(title).slice(0,200),
        ...(typeof jobId === "string" && jobId.trim() ? { jobId } : {}) });
    } else if (!Array.isArray(value) && !value._type && typeof value._id === "string" && !sources.has(value._id)) {
      unresolved.add(value._id);
    }
  ...
  return [...unresolved].filter(id => !sources.has(id));
}
...
export async function connectContext({ signal, env = process.env, fetchImpl = fetch, createClient = createMCPClient,
  createSourceClient = createContentClient } = {}) {
  ...
  try {
    ...
    const tools = Object.fromEntries(["groq_query", "schema_explorer"].filter(name => discovered[name]?.execute).map(name => {
      ...
      return [name, { ...original, execute: async (input, options) => {
        ...
        const unresolved = collectSources(result, sources);
        if (unresolved.length) {
          // Context projections can include _id while omitting _type. Resolve only those IDs
          // against the app's published, allowed documents before showing source metadata.
          try {
            const docs = await createSourceClient(env).fetch(SOURCE_METADATA_QUERY,
              {ids:unresolved,types:CHAT_TYPES}, {signal});
            collectSources({structuredContent:docs}, sources);
          } catch { /* The answer can continue when source metadata is unavailable. */ }
        }
        return result;
      } }];
    }));
