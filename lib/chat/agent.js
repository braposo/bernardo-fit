...
export function createChatAgent({ route, context, ref, settings, makeModel = chatLanguageModel, record = recordUsage }) {
  ...
  return new ToolLoopAgent({
    ...
    instructions: [settings.assistantInstructions, settings.contextInstructions, context.initialContext,
      "The app separately displays retrieved sources. Do not add a Sources, References, or citations section to your answer. Do not print Sanity document IDs, revision IDs, migration IDs, or raw internal reference objects. Refer to source titles naturally when they help explain a claim; never invent a link."].join("\n\n"),
    tools: context.tools,
