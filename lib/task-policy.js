...
export function assertStorageDispatch(env = process.env) {
  ...
  // Preview apps must not silently use production workers with a different store.
  if (preview && env.TRIGGER_SECRET_KEY?.startsWith('tr_prod_')) fail();
  if (env.SANITY_CONTENT_ENABLED !== '1') return;
  if (env.SANITY_WORKERS_READY !== '1' || preview && (!env.TRIGGER_PREVIEW_BRANCH || !branchKey)
