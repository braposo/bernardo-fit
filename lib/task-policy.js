...
export function assertStorageDispatch(env = process.env) {
  ...
  if (env.SANITY_WORKERS_READY !== '1' || env.VERCEL_ENV === 'preview' && (!env.TRIGGER_PREVIEW_BRANCH || !env.TRIGGER_SECRET_KEY?.startsWith('tr_preview_'))) {
    throw Object.assign(new Error('The Sanity workers for this branch are not connected yet. Job editing is available; generation will be available once the matching Trigger environment is deployed.'),{status:503,code:'SANITY_WORKERS_NOT_READY'});
  }
}
