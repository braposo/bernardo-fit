...
export function assertStorageDispatch(env = process.env) {
  ...
  if (env.SANITY_WORKERS_READY !== '1' || env.VERCEL_ENV === 'preview' && (!env.TRIGGER_PREVIEW_BRANCH || !branchKey)
    || env.VERCEL_ENV === 'production' && development) {
    throw Object.assign(new Error('The Sanity workers for this branch are not ready yet. Job editing is available; generation will be available once the matching Trigger worker is fully configured and running.'),{status:503,code:'SANITY_WORKERS_NOT_READY'});
  }
}
