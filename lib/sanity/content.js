import { createContentClient, SANITY_PROJECT_ID, SANITY_DATASET, sanityStorageEnabled } from "./client.js";
...
export async function contentStatus(client = createContentClient()) {
  ...
  return { projectId: SANITY_PROJECT_ID, dataset: SANITY_DATASET, connected: true,
    analysisSource: process.env.SANITY_ANALYSIS_ENABLED === "1" ? "sanity" : "code-baseline",
    mode: sanityStorageEnabled() ? "connected" : "prepared", activeAppStorage: sanityStorageEnabled() ? "sanity" : "existing", counts };
}
