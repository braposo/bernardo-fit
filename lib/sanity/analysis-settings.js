...
const baseline = freeze({ ...structuredClone(DEFAULT_SETTINGS), fingerprint: hash(DEFAULT_SETTINGS), revision: "code-baseline" });
...
export function settingsFromDocument(doc) {
  ...
  return freeze({ ...settings, fingerprint: hash(settings), revision: doc._rev || "published" });
}
...
export async function loadAnalysisSettings(env = process.env, client) {
  ...
  try { return settingsFromDocument(await (client || createContentClient(env)).fetch(ANALYSIS_SETTINGS_QUERY)); }
  catch (error) {
