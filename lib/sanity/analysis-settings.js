...
const fitSettings = settings => ({candidate:settings.texts.candidateProfile, dimensions:settings.dimensions,
  questions:Object.fromEntries(DEFAULT_SETTINGS.dimensions.flatMap(d=>[d.id,d.id+'Known']).concat(['posting','constraint']).map(k=>[k,settings.questions[k]]))});
const baseline = freeze({ ...structuredClone(DEFAULT_SETTINGS), fingerprint: hash(DEFAULT_SETTINGS), revision: "code-baseline", legacyScoringCompatible:true, legacyFitCompatible:true });
// Hash of the scoring text assembled from the original migration snapshot.
// Interview/motivation notes and answer-writing guidance never enter Jev scoring.
const IMPORTED_SCORING_TEXT_SIGNATURE='965c45bf688fe9b33ec35951b4a720495354d887b32c50fd2158bce60633f1be';
...
export function settingsFromDocument(doc) {
  ...
  return freeze({ ...settings, fingerprint, revision: doc._rev || "published",legacyScoringCompatible:fingerprint===baseline.fingerprint,
    legacyFitCompatible:hash(fitSettings(settings))===hash(fitSettings(baseline)) });
}
...
export async function loadAnalysisSettings(env = process.env, client) {
  ...
  try {
    ...
    delete mutable.fingerprint;delete mutable.revision;delete mutable.legacyScoringCompatible;delete mutable.legacyFitCompatible;
    return freeze({...mutable,fingerprint:hash(mutable),revision:`${settings.revision}:${candidate._rev}`,
      ...
      legacyFitCompatible:settings.legacyFitCompatible && hash(mutable.texts.candidateProfile)===IMPORTED_SCORING_TEXT_SIGNATURE});
  }
  catch (error) {
