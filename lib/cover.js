...
export function buildCoverPrompt({ report, fitUrl, instructions }) {
  const stable = `You are writing a cover letter as Bernardo Raposo, in the first person. It is sent to the company, so it is public-facing writing, not a private note.
...
If you know the company name, address the salutation to the team by name, for example "Dear Sanity team,". Otherwise keep "Dear Hiring Team,".`;
...
export async function runCoverLetter({ report, fitUrl, instructions, model, ref }) {
  const { stable, volatile } = buildCoverPrompt({ report, fitUrl, instructions });
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    const { text } = await complete({
      ...
      system: { stable, volatile: volatile + addendum },
