...
export async function editQuestion(id, edit) {
  ...
  return mutateJob(id, job => {
    ...
    if (edit.remove) return { questions: questions.filter(q => q.id !== edit.id), briefFingerprint: "" };
    ...
    return { questions: existing ? questions.map(q => q.id === next.id ? next : q) : [...questions, next], briefFingerprint: "" };
  });
}
