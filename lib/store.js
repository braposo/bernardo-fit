...
import { LEGACY_MODEL } from "./models.js";
import { auditKey, auditEvent, jobMutationEvents, appendMemoryAudit } from "./job-audit.js";
import { sanityStorageEnabled } from "./sanity/client.js";
import { repository } from "./sanity/repository.js";
...
const cacheKey = report => legacyCacheKey(report) + ":" + digest([report?.model || LEGACY_MODEL, report?.generation || null]);

function sourceOf(report) {
  return {
    hash: hashJD(report.job_description || ""),
    length: (report.job_description || "").length,
    model: report.model || LEGACY_MODEL,
...
async function changeReport(id, mutate, { create = false, expectedRevision } = {}) {
  if (sanityStorageEnabled()) return repository().changeReport(id, mutate, {create, expectedRevision});
  for (let attempt = 0; attempt < 12; attempt++) {
    ...
    if (expectedRevision !== undefined && revision !== expectedRevision) {
      throw Object.assign(new Error("The analysis changed while work was running."), { status: 409, code: "REPORT_CHANGED" });
    }
    ...
    const added = nextVersions.filter(v => !versions.some(old => old.vid === v.vid));
    const active = nextVersions.find(v => v.active);
    const events = added.map(v => auditEvent("document.generated", "Fit analysis generated", [v.model, v.vid].filter(Boolean).join(" · ")));
    if (active && active.vid !== versions.find(v => v.active)?.vid) events.push(auditEvent("document.published", "Fit analysis published live", [active.model, active.vid].filter(Boolean).join(" · ")));
    if (!nextReport) events.push(auditEvent("document.deleted", "Fit analysis deleted"));
    ...
    const keys = ["fit:" + id, versionKey(id), "fitrev:" + id, sourceKey(id), INDEX_KEY,
      cacheKey(report), legacyCacheKey(report), cacheKey(nextReport), legacyCacheKey(nextReport), auditKey("report", id)];
    let saved;
    if (client) {
      saved = await client.eval(REPORT_WRITE, keys, [
        ...
        JSON.stringify(events.map(event => JSON.stringify(event))),
      ]);
    } else {
      ...
      if (saved) {
        ...
        appendMemoryAudit(keys[9], events);
      }
    }
...
export async function saveReportWithId(id, report, internal, { vid = id, versionInstructions = "" } = {}) {
  const existing = await getReport(id);
  if (existing) return id;
  await addReportVersion(id, report, internal, { create: true, vid, versionInstructions });
  return id;
}

export async function getReport(id) {
  if (sanityStorageEnabled()) return repository().getReport(id);
  return hasKV ? (await (await kv()).get("fit:" + id)) || null : memGet("fit:" + id) || null;
}

export async function getSharedReport(id) {
  return sanityStorageEnabled() ? repository().getSharedReport(id) : getReport(id);
}

export async function getReportRevision(id) {
  if (sanityStorageEnabled()) return repository().getReportRevision(id);
  return Number(hasKV ? await (await kv()).get("fitrev:" + id) : memGet("fitrev:" + id)) || 0;
}

export async function listReportVersions(id) {
  if (sanityStorageEnabled()) return repository().listReportVersions(id);
...
export async function addReportVersion(id, report, internal, options = {}) {
  ...
  return changeReport(id, (_, versions) => {
    const entry = {
      ...
      versionInstructions: options.versionInstructions || "",
    };
    return {
      report,
...
export async function listReportIds({ offset = 0, limit = 100 } = {}) {
  if (sanityStorageEnabled()) return (await repository().listReports()).slice(offset, offset + limit).map(r=>r.id);
...
export async function listReports({ offset = 0, limit = 20 } = {}) {
  if (sanityStorageEnabled()) { const all=await repository().listReports();return {reports:all.slice(offset,offset+limit),total:all.length}; }
...
export async function getReportSources(ids) {
  ...
  if (sanityStorageEnabled()) {
    const pairs=await Promise.all(unique.map(async id=>{const report=await getReport(id);return [id,report?sourceOf(report):null];}));
...
export const JOB_STAGES = [
  ...
  // process did. Kept apart from rejected and not_interested so the pipeline counts
  // stay honest about what actually happened.
  "expired",
];

export function normaliseJobStage(stage) {
  return stage === "not_a_fit" ? "not_interested" : stage;
}
export const ARCHIVE_ON_STAGE = ["expired", "rejected", "not_interested"];

// Read legacy stages consistently; the next write persists the new name.
function readJobRecord(job, id) {
  return job ? { ...job, id, stage: normaliseJobStage(job.stage) } : null;
}
...
async function listAllJobs() {
  if (sanityStorageEnabled()) return repository().listJobs();
  if (hasKV) {
    const store = await kv();
    const ids = await store.zrange(JOBS_INDEX, 0, -1, { rev: true });
    if (!ids.length) return [];
    const jobs = await store.mget(...ids.map((id) => `job:${id}`));
    return ids.map((id, i) => readJobRecord(jobs[i], id)).filter(Boolean);
  }
  return memJobIndex
    .slice()
...
export async function getJob(id) {
  if (sanityStorageEnabled()) return repository().getJob(id);
  if (hasKV) {
    const store = await kv();
...
function jobRecord(job, revision) {
  ...
  return {
    ...
    auditLegacy: !!job.auditLegacy,
    auditReportIds: job.auditReportIds || [],
    revision,
    ...
    jevAssessment: job.jevAssessment || null,
    overviewSummary: job.overviewSummary || null,
    jevRun: normaliseRun(job.jevRun),
    ...
    coverRun: normaliseRun(job.coverRun),
    analysisRun: normaliseRun(job.analysisRun),
    researchId: job.researchId || "",
    researchAt: job.researchAt || "",
    researchModel: job.researchModel || "",
    researchSourceCount: Number(job.researchSourceCount) || 0,
    researchPartial: !!job.researchPartial,
    researchFingerprint: job.researchFingerprint || "",
    briefId: job.briefId || "",
    briefAt: job.briefAt || "",
    briefModel: job.briefModel || "",
    briefFingerprint: job.briefFingerprint || "",
    briefStage: job.briefStage || "",
    researchRun: normaliseRun(job.researchRun),
    briefRun: normaliseRun(job.briefRun),
    prepareRun: normaliseRun(job.prepareRun),
    ...
    stage: JOB_STAGES.includes(normaliseJobStage(job.stage)) ? normaliseJobStage(job.stage) : "new",
...
function normaliseRun(run) {
  return run && typeof run === "object" ? {
    requestId: run.requestId || "", runId: run.runId || "", fingerprint: run.fingerprint || "",
    model: run.model || "",
...
export async function mutateJob(id, mutate, { create = false, expectedRevision, expectedReportRevision } = {}) {
  const requestedMutation = mutate;
  mutate = current => {
    const patch = requestedMutation(current);
    if (!patch) return patch;
    const next = { ...patch };
    // Apply the same archive policy to API, worker and import writes. An
    // explicit restore wins; unrelated edits must not rearchive restored jobs.
    if (ARCHIVE_ON_STAGE.includes(normaliseJobStage(next.stage)) && next.archived === undefined) next.archived = true;
    if (next.archived === true) next.archivedAt = current.archivedAt || next.archivedAt || new Date().toISOString();
    if (next.archived === false) next.archivedAt = "";
    return next;
  };
  if (sanityStorageEnabled()) return repository().changeJob(id, mutate, {create, expectedRevision, expectedReportRevision}, jobRecord);
  for (let attempt = 0; attempt < 12; attempt++) {
    ...
    if (record) {
      record.auditStartedAt = current?.auditStartedAt || new Date().toISOString();
      record.auditLegacy = current ? !current.auditStartedAt || !!current.auditLegacy : false;
      record.auditReportIds = [...new Set([...(current?.auditReportIds || []), current?.fitReportId, record.fitReportId].filter(Boolean))];
    }
    const events = jobMutationEvents(current, record, revision + 1);
    if (current && !current.auditStartedAt) events.unshift(auditEvent("job.created", "Added to pipeline", "From the saved creation date", { id: "legacy-creation", at: current.createdAt }));
    ...
    if (hasKV) {
      ...
      saved = await (await kv()).eval(JOB_WRITE, [`job:${id}`, JOBS_INDEX, DISMISSED, reportRevisionKey, auditKey("job", id)],
        [revision, record ? JSON.stringify(record) : "", score, id,
          expectedReportRevision ? expectedReportRevision.revision : "", JSON.stringify(events.map(event => JSON.stringify(event)))]);
      if (Number(saved) === -1) {
      ...
      if (saved) {
        appendMemoryAudit(auditKey("job", id), events);
        if (record) {
...
export async function editQuestion(id, edit) {
  ...
  return mutateJob(id, job => {
    ...
    if (edit.remove) return { questions: questions.filter(q => q.id !== edit.id), ...(existing?.a ? { briefFingerprint: "" } : {}) };
    ...
    if (existing && JSON.stringify(existing) === JSON.stringify(next)) return undefined;
    return { questions: existing ? questions.map(q => q.id === next.id ? next : q) : [...questions, next],
      ...(existing?.a && (existing.q !== next.q || existing.a !== next.a) ? { briefFingerprint: "" } : {}) };
  });
}
...
export async function dismissedReports() {
  if (sanityStorageEnabled()) return repository().dismissedReports();
  if (hasKV) {
    const store = await kv();
...
export async function undismissReport(reportId) {
  ...
  if (sanityStorageEnabled()) return repository().undismissReport(reportId);
  if (hasKV) {
    const store = await kv();
...
export function findExistingJobIn(jobs, { externalId, threadId, sourceUrl, company, role }) {
  if (externalId) {
...
export async function findExistingJob(candidate) {
  ...
  return findExistingJobIn(await listAllJobs(), candidate);
}

export async function createJobIfAbsent(id, job) {
  return mutateJob(id, (current) => current.id ? undefined : job, { create: true });
}
...
export async function trackEvent(reportId, event) {
  if (typeof reportId !== "string" || !reportId || reportId.length > 200 || !TRACKED_EVENTS.includes(event)) return false;
  const now = new Date().toISOString();
  const entry = auditEvent("engagement." + event, { view: "Fit page viewed", copy_link: "Fit link copied", cv_download: "CV download requested" }[event], "", { at: now, actor: "visitor" });
  if (hasKV) {
    const store = await kv();
    await store.pipeline().hincrby(`stats:${reportId}`, event, 1)
      .zadd(auditKey("report", reportId), { score: Date.parse(now), member: JSON.stringify(entry) })
      .hset(`stats:${reportId}`, { lastAt: now }).hsetnx(`stats:${reportId}`, "firstAt", now).exec();
  } else {
    appendMemoryAudit(auditKey("report", reportId), [entry]);
...
export async function findReportByHash(jd, options) {
  ...
  if (sanityStorageEnabled()) {
    const found=(await repository().listReports()).find(r=>hashJD(r.job_description)===hashJD(jd) && (!options || cacheKey(r)===key));
    if (!found) return null;
    const {id,...report}=found;return {id,report};
  }
