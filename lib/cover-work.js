...
async function attachArtifact({ jobId, requestId, fingerprint, model, artifact, versionCount, versionInstructions = "" }) {
  ...
  const patch = (current, canAttach) => {
    attached = canAttach && selected && current.coverRun?.requestId === requestId &&
      current.coverRun?.fingerprint === fingerprint && coverFingerprint(withVersionInstructions(current, versionInstructions), report, model) === fingerprint;
    if (!attached) {
...
export async function executeCoverWork({ jobId, requestId, fingerprint, model, origin, versionInstructions = "" }) {
  ...
  if (!persisted) {
    ...
    const saved = await saveCoverArtifact(jobId, {
      vid: requestId,
      versionInstructions: job._versionInstructions || "",
  ...
  const attached = await attachArtifact({
    jobId, requestId, fingerprint, model: wantedModel, artifact: persisted, versionCount, versionInstructions,
  });
  return {
