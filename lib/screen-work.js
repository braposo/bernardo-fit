...
import { briefFingerprint, researchFingerprint, researchInputsAreCurrent } from "./generation-fingerprint.js";
...
export function researchIsReusable(job, research, now = Date.now(), model) {
  return !!research && (!model || research.model === resolveModel(model)) && researchInputsAreCurrent({ ...job, _versionInstructions: "" }) &&
...
export async function executeResearchWork({ jobId, requestId, fingerprint, model = PUBLIC_MODEL, versionInstructions = "" }) {
  const job = withVersionInstructions(await getJob(jobId), versionInstructions);
  ...
  if (!artifact) artifact = await saveScreenArtifact("research", jobId, requestId,
    { ...await runResearch({ job, model, ref: requestId }), versionInstructions: job._versionInstructions || "" });
  let attached = false;
  await mutateJob(jobId, (current) => {
    attached = current.researchRun?.requestId === requestId && current.researchRun?.fingerprint === fingerprint &&
      researchFingerprint(withVersionInstructions(current, versionInstructions)) === fingerprint;
    return attached ? {
      ...
      researchFingerprint: researchFingerprint(current),
...
export async function executeBriefWork({ jobId, requestId, fingerprint, model, versionInstructions = "" }) {
  const job = withVersionInstructions(await getJob(jobId), versionInstructions);
  if (!job) throw Object.assign(new Error("Job not found"), { status: 404, abort: true });
  ...
  if (!artifact) artifact = await saveScreenArtifact("brief", jobId, requestId,
    { ...await runBrief({ job, report, research, model: wantedModel, ref: requestId }), versionInstructions: job._versionInstructions || "" });
  ...
  const patch = (current, validReport) => {
    attached = validReport && current.briefRun?.requestId === requestId && current.briefRun?.fingerprint === fingerprint &&
      current.researchId === research.id && briefFingerprint(withVersionInstructions(current, versionInstructions), report, research) === fingerprint;
    return attached ? {
