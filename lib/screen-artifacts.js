...
export async function getScreenArtifact(kind, jobId, id) {
  ...
  if (sanityStorageEnabled()) return repository().getArtifact(kind,jobId,id);
...
export async function saveScreenArtifact(kind, jobId, id, artifact) {
  ...
  if (sanityStorageEnabled()) return repository().saveArtifact(kind,jobId,id,value);
...
export async function listScreenArtifacts(jobId, kind) {
  ...
  if (sanityStorageEnabled()) return repository().listArtifacts(kind,jobId);
  let keys;
...
export async function deleteScreenArtifacts(jobId) {
  if (sanityStorageEnabled()) return (await repository().deleteArtifacts('research',jobId)) + (await repository().deleteArtifacts('brief',jobId));
  if (hasKV) {
    const client = await kv();
