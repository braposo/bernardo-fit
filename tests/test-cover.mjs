...
const { getCoverArtifact, listCoverVersions } = await import(lib + "cover-artifacts.js");
...
res = mockRes();
await coverHandler({ method: "POST", headers: auth, body: { id: "nope", requestId: "request00" } }, res);
check("kill switch pauses new dispatch", res.statusCode === 503, res.body);
await saveRunReceipt({ kind: "cover", jobId: "known", requestId: "recovery01", runId: "run_known", fingerprint: "f" });
res = mockRes();
await coverHandler({ method: "POST", headers: auth, body: { id: "known", requestId: "recovery01" } }, res);
check("kill switch still recovers accepted dispatch", res.statusCode === 202 && res.body.runId === "run_known", res.body);
...
check("saves only an artifact pointer on the row", !!saved.coverLetterId && saved.coverLetter === null);
check("moves version bodies out of the row", saved.coverLetterVersions.length === 0);
...
check("the existing live letter remains", afterLate.coverLetterId === saved.coverLetterId, afterLate.coverLetterId);
check("the late draft is retained but inactive", (await listCoverVersions(afterLate)).some((v) => v.vid === "request03" && !v.active));
