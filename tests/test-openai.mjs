...
for (const model of ["gpt-5.6-sol", "gpt-6-astra"]) await test(model + " supports every per-job generator", async () => {
  ...
  mock(JSON.stringify({ salutation: "Dear team,", paragraphs: [{ lead: true, text: "I have built this kind of platform before." }] }));
  await runCoverLetter({ model, report: { job_description: "Engineer" }, fitUrl: "https://example.com" });
  assert.equal(calls[0].body.model, model);
  mock("I led the team."); await runAnswer({ model, question: "Describe your leadership?" });
  assert.equal(calls[0].body.model, model);
  mock(JSON.stringify({ opening: "Hello" }));
  await runBrief({ model, job: { company: "Sanity", role: "Engineer" }, report: {}, research: { sources: [] } });
  assert.equal(calls[0].body.model, model);
  mock(JSON.stringify({ summary: ["Company evidence"], sources: [{ title: "Company", url: "https://example.com" }] }));
  const research = await runResearch({ model, job: { company: "Sanity", role: "Engineer" }, ref: "research-" + model });
  assert.equal(calls[0].body.model, model); assert.equal(calls[0].body.tools[0].type, "web_search");
  assert.equal(research.model, model); assert.ok(research.sources.length);
});
