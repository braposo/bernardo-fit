...
const shared = { SANITY_CONTENT_ENABLED:'1', SANITY_ANALYSIS_ENABLED:'1', SANITY_WORKERS_READY:'1', SANITY_WORKER_ENV:'production', VERCEL_ENV:'preview', TRIGGER_SECRET_KEY:'tr_prod_test', SANITY_WRITE_TOKEN:'fixture' };
test(()=>assert.doesNotThrow(()=>assertStorageDispatch(shared)));
for (const patch of [{SANITY_CONTENT_ENABLED:'0'},{SANITY_ANALYSIS_ENABLED:'0'},{SANITY_WRITE_TOKEN:''},{KV_NAMESPACE:'preview'},{TRIGGER_PREVIEW_BRANCH:'branch'},{TRIGGER_SECRET_KEY:'tr_dev_test'}]) {
  test(()=>assert.throws(()=>assertStorageDispatch({...shared,...patch}),/same content storage/));
}
